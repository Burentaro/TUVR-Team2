...
public class BowlingBall : MonoBehaviour
{
    ...
    {
        GameObject[] Bowling = inventoryItems;
        ...
        if (itemNumber == 0 && k == 1)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                for (int j = 0; j < 4 - i; j++)
                {
                    Bowling[m] = Instantiate<GameObject>(pinPrefab, dispensePosition.position + new Vector3((float)(-1.5 * 0.25), 0.0f, 8.7f) + new Vector3((float)((j + 0.5 * i) * 0.25), (float)(0.5 * 0.25), (float)(-i * 0.5 * 0.25) * (float)Math.Sqrt(3)), dispensePosition.rotation);
                    m++;
                }
            }
            k = 0;
        ...
        if (itemNumber == 1 && k == 1)
        {
            Bowling[0] = Instantiate<GameObject>(bballPrefab, dispensePosition.position + new Vector3(-1.0f, (float)(0.5 * 0.5), -11.5f),dispensePosition.rotation);
            k = 0;
        }
